"""Tests for screening_abstracts.export_xlsx."""
...
# --- short_work_id + _unique_sheet_name ---
...
def test_export_full_set(tmp_path: Path) -> None:
    ...
    # Index row count includes header.
    index = wb["Index"]
...
def test_export_subset_preserves_verdict_order(tmp_path: Path) -> None:
...
def test_export_subset_oversize_emits_full_set(tmp_path: Path) -> None:
...
def test_export_sentinel_relevance_score_is_none(tmp_path: Path) -> None:

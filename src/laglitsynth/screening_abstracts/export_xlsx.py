"""XLSX export of stage 3 verdicts for per-work spot-checking.

One workbook with an ``Index`` sheet plus one tab per included work.
Per-work tabs use a vertical ``Field | Value`` layout so abstract and
``raw_response`` wrap vertically without horizontal scrolling.
``--n-subset`` + ``--subset-seed`` draw a reproducible random sample;
``n_subset >= len(verdicts)`` (or unset) emits the full set in
verdict order.
"""
...
def build_subparser(
    ...
    parser = subparsers.add_parser(
        ...
        help="Export stage 3 verdicts + catalogue to a per-work XLSX workbook.",
    )
    parser.add_argument("--verdicts", type=Path, required=True, help="verdicts.jsonl path")
    parser.add_argument(
        "--catalogue", type=Path, required=True, help="dedup catalogue JSONL path"
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help="Output XLSX path (default: <verdicts parent>/review.xlsx)",
    )
    parser.add_argument(
        "--n-subset",
        type=int,
...
def run(args: argparse.Namespace) -> None:
    ...
    output_path: Path = (
        args.output if args.output is not None else verdicts_path.parent / "review.xlsx"
    )

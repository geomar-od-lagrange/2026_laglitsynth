...
def build_work_sheet(
    ...
    *,
    ...
    ``EligibilityMeta.llm`` so the reviewer can audit the LLM's run; it is
    ...
    ws.cell(row=18, column=2, value=llm_meta.model if llm_meta else "")
    ...
    ws.cell(row=19, column=2, value=llm_meta.temperature if llm_meta else None)

    llm_sha_label = ws.cell(row=20, column=1, value="llm_prompt_sha256")
    llm_sha_label.font = _BOLD
    llm_sha_label.alignment = top_aligned
    ws.cell(row=20, column=2, value=llm_meta.prompt_sha256 if llm_meta else "")
...
def _load_meta(meta_path: Path | None) -> tuple[str, LlmMeta | None]:
    """Return ``(criterion, llm_meta)`` from an eligibility-meta.json file.
    ...
    if meta_path is None or not meta_path.exists():
        ...
        return ("<eligibility criterion not available>", None)

    meta = EligibilityMeta.model_validate_json(meta_path.read_text())
